#!/usr/bin/env python3
"""
TF-IDF baselines for NormDef-FR v0.6 candidate validation.

Task:
    Given an automatically extracted candidate (term + definition + context),
    predict whether it is acceptable:
        1 = accepted  (valid_definition or partial)
        0 = invalid

Inputs:
    data/experiments/v0_6/splits/candidate_validation/train.jsonl
    data/experiments/v0_6/splits/candidate_validation/dev.jsonl
    data/experiments/v0_6/splits/candidate_validation/test.jsonl

Outputs:
    data/experiments/v0_6/results/candidate_validation_tfidf_metrics.json
...
Usage:
    python scripts/train_tfidf_candidate_validator.py
"""
...
OUTPUT_METRICS = OUTPUT_DIR / "candidate_validation_tfidf_metrics.json"
OUTPUT_PREDICTIONS = OUTPUT_DIR / "candidate_validation_tfidf_predictions.jsonl"
...
def evaluate_model(model: Pipeline, x: list[str], y: list[int]) -> dict[str, Any]:
    ...
    return {
        ...
        "accepted_f1": f1_score(y, pred, pos_label=1, zero_division=0),
        "invalid_f1": f1_score(y, pred, pos_label=0, zero_division=0),
        "labels": {
            "0_invalid": {
...
def main() -> int:
    ...
    all_metrics: dict[str, Any] = {
        ...
        "notes": [
            ...
            "This task models the automatic filtering of candidates produced by the rule-based extractor.",
        ],
    }
    ...
    print(
        f"- {best[0]}: macro-F1={best[1]['test']['macro_f1']:.3f}, "
        f"accepted-F1={best[1]['test']['accepted_f1']:.3f}, "
        f"invalid-F1={best[1]['test']['invalid_f1']:.3f}"
    )
